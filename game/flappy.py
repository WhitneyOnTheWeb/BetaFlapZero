...
class Environment:
    ...
    def reset(self):
        ...
        self.__init__(difficulty=mode[idx])
    ...
    def step(self, action):
        ...
        * Based on action and current position, and acceleration '''
        ...
        if self.playery <= 0:
            self.playery = 0
        self.playery_mid = self.playery + (PLAYER_H // 2)
        self.player_btm = self.playery + PLAYER_H

        '''---Shift pipes to the left---'''
        for uPipe, lPipe in zip(self.upperPipes, self.lowerPipes):
            uPipe['x'] += self.pipeVelX
            uPipe['x_mid'] += self.pipeVelX
            uPipe['x_right'] += self.pipeVelX
            lPipe['x'] += self.pipeVelX
            lPipe['x_mid'] += self.pipeVelX
            lPipe['x_right'] += self.pipeVelX
            for p in np.append(self.upperPipes, self.lowerPipes):
                left, right = self.get_pipe_corners(p)
                p.update({
                    'corners': {
