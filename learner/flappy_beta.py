...
class NeuralNet:
    def __init__(self,
        ...
        with tf.device(gpu):
            self.inputs, self.outputs = self._create_model(S, A, H, lr)
...
class BetaFlapDQN(DQNAgent):
    def __init__(self, inputs, buffer, sess, **kwargs):
        self.util = Utility()
        self.sess = sess
    ...
    def backward(self, reward, terminal):
        ...
        and self.step % self.train_interval == 0:
            ...
            if type(metrics) is list: 
                ...
                metrics += self.policy.metrics          # this line missing :(
            #else:
